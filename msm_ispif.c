	...
	struct ispif_irq_status *out, enum msm_ispif_vfe_intf vfe_id)
{
	...
	if (out[vfe_id].ispifIrqStatus0 &
		...
		printk("ispif_process_irq : PIX0 timestamp %ld\n",timestamp);
	...
	if (out[vfe_id].ispifIrqStatus0 &
		...
		timestamp = ktime_get_real_ns();
		printk("ispif_process_irq : RDI0 timestamp %ld\n",timestamp);
	}
	if (out[vfe_id].ispifIrqStatus1 &
		...
		timestamp = ktime_get_real_ns();
		printk("ispif_process_irq : RDI1 timestamp %ld\n",timestamp);
	}
	if (out[vfe_id].ispifIrqStatus2 &
		...
		timestamp = ktime_get_real_ns();
		printk("ispif_process_irq : RDI2 timestamp %ld\n",timestamp);
	}
}
...
static irqreturn_t msm_io_ispif_irq(int irq_num, void *data)
{
	...
	s64 timestamp = 0;
	timestamp = ktime_get_real_ns();
	printk("msm_io_ispif_irq : formal timestamp %ld\n",timestamp);
